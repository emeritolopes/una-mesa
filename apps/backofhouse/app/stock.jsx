...
function StockItemModal({ item, onSave, onClose }) {
  const isEdit = !!item;
  const [f, setF] = useState(item ? { ...item, qty: String(item.qty), par: String(item.par), cost: String(item.cost) } : { name: '', category: 'Carne', unit: 'kg', qty: '', par: '', cost: '' });
  ...
  const submit = () => {
    ...
    onSave({ ...(isEdit ? item : { id: 'sk' + Date.now() }), name: f.name.trim(), category: f.category, unit: f.unit, qty: Number(f.qty) || 0, par: Number(f.par) || 0, cost: Number(f.cost) || 0 });
  };
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl w-full max-w-sm overflow-hidden shadow-xl" onClick={e => e.stopPropagation()}>
        <div className="px-5 py-4 border-b border-black/7 flex items-center justify-between">
          <div className="font-['Syne'] text-base font-black text-gray-900">{isEdit ? 'Editar artículo' : 'Nuevo artículo'}</div>
        ...
        <div className="p-5 pt-0 flex gap-2">
          ...
          <button onClick={submit} className="flex-1 py-2.5 rounded-xl bg-brand text-white text-xs font-bold hover:bg-brand/90 transition">{isEdit ? 'Guardar cambios' : 'Añadir artículo'}</button>
        </div>
      </div>
    </div>
  );
...
function Stocktake() {
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden">
      ...
      {editItem && <StockItemModal item={editItem} onSave={saveEdit} onClose={() => setEditItem(null)} />}
      {confirmDel && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setConfirmDel(null)}>
          <div className="bg-white rounded-2xl w-full max-w-sm overflow-hidden shadow-xl" onClick={e => e.stopPropagation()}>
            <div className="bg-red-50 px-5 py-4 flex items-center gap-3 border-b border-red-100">
              ...
              <div>
                <div className="font-['Syne'] text-base font-black text-red-700">¿Eliminar artículo?</div>
            ...
            <div className="p-5 text-sm text-gray-600 leading-relaxed">
              Vas a eliminar <span className="font-semibold text-gray-900">{confirmDel.name}</span> del inventario. ¿Estás seguro?
            </div>
            <div className="p-5 pt-0 flex gap-2">
              ...
              <button onClick={() => doRemove(confirmDel.id)} className="flex-1 py-2.5 rounded-xl bg-red-600 text-white text-xs font-bold hover:bg-red-700 transition">Sí, eliminar</button>
            </div>
          </div>
        </div>
      )}
                      ...
                      <tr key={s.id} className={`border-t border-black/5 hover:bg-gray-50 transition group ${st.row}`}>
                        ...
                        <td className="pr-4">
                          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition">
                            <button onClick={() => setEditItem(s)} title="Editar" className="w-7 h-7 rounded-lg text-gray-400 hover:text-brand hover:bg-brand/10 flex items-center justify-center"><i className="ti ti-pencil text-sm" /></button>
                            ...
                            <button onClick={() => setConfirmDel(s)} title="Eliminar" className="w-7 h-7 rounded-lg text-gray-300 hover:text-red-500 hover:bg-red-50 flex items-center justify-center"><i className="ti ti-trash text-sm" /></button>
                          </div>
                        </td>
                      </tr>
                    );
