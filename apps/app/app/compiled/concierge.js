...
const CC_T = {
  es: {
    ...
    examples: ['Quiero una mesa para 5 en Madrid para veganos', 'Cena romántica con terraza para dos', 'Algo de marisco para 6, sin gastar mucho', 'El mejor sitio de arroces para comer hoy'],
    eyebrow: 'Conserje IA',
    headingA: 'Dime qué te apetece y ',
    headingHl: 'yo reservo',
    headingB: '.',
  ...
  en: {
    ...
    examples: ['I want a table for 5 in London, vegan-friendly', 'Romantic dinner with a terrace for two', 'Something seafood-y for 6, without breaking the bank', 'The best rice place for lunch today'],
    eyebrow: 'AI Concierge',
    headingA: 'Tell me what you fancy and ',
    headingHl: 'I\'ll book it',
    headingB: '.',
...
function parseIntent(text) {
  const t = ' ' + text.toLowerCase().normalize('NFD').replace(/[̀-̃̈]/g, '') + ' '; // strip accents (keep ñ)
  let party = null;
  const numWords = {
    un: 1,
    una: 1,
...
function matchRestaurants(intent) {
  ...
  const scored = data.map(r => {
    ...
    if (intent.dietary.length) {
      if (isVeg) score += 0.7;else {
        dietOk = false;
      }
    }
    intent.cuisines.forEach(c => {
      if (hay.includes(c.toLowerCase())) score += 0.45;
    });
    intent.vibes.forEach(v => {
      if (hay.includes(v.toLowerCase())) score += 0.3;
    });
...
function templatedReply(intent, picks) {
  ...
  if (intent.time && intent.time !== 'noche' && intent.time !== 'comida') bits.push(CC_T.atTime(intent.time));else if (intent.time) bits.push(intent.time === 'noche' ? CC_T.forDinner : CC_T.forLunch);
  const otherCity = intent.city && intent.city !== 'vigo';
  let head;
  if (otherCity) head = CC_T.otherCityHead;else head = bits.length ? CC_T.matchHead(bits.join(', ')) : CC_T.genericHead;
...
function ConciergeScreen({
  ...
  async function ask(text) {
    ...
    try {
      ...
      setBusy(false);
      if (json.action) {
        const rest = (window.UM_DATA || []).find(r => r.name.toLowerCase().includes(json.action.restaurant_name.toLowerCase()));
        if (rest) {
      ...
      setMsgs(m => [...m, {
        who: 'ai',
        text: templatedReply(intent, picks),
        picks,
        intent
      }]);
    }
  ...
  return React.createElement('div', {
    className: 'view cc-view'
  }, React.createElement('div', {
    className: 'wrap cc-wrap'
  }, /* ── Header ── */
  React.createElement('div', {
    className: 'cc-head'
  }, React.createElement('span', {
    className: 'eyebrow'
  }, CC_T.eyebrow), React.createElement('h1', {
    className: 'display'
  }, CC_T.headingA, React.createElement('span', {
    className: 'hl'
  }, CC_T.headingHl), CC_T.headingB), React.createElement('p', null, CC_T.sub)), /* ── Chat panel ── */
  React.createElement('div', {
    className: 'cc-panel'
  }, /* Scroll area */
  React.createElement('div', {
    ...
    className: 'tdot'
  }))) : null), /* Suggestion chips */
  React.createElement('div', {
    className: 'cc-examples'
  }, examples.map((ex, i) => React.createElement('button', {
    key: i,
    className: 'chip',
    onClick: () => ask(ex),
    disabled: busy
  }, ex))), /* Input form — editorial border-bottom, icon send button */
...
function ConciergeMsg({
  ...
  if (m.who === 'me') return React.createElement('div', {
    className: 'cc-msg me'
  }, React.createElement('div', {
    className: 'cc-bub'
  }, m.text));
  ...
  return React.createElement('div', {
    ...
    className: 'cc-bub'
  }, m.text), m.picks && m.picks.length ? React.createElement('div', {
    className: 'cc-results'
  }, m.picks.map((r, i) => React.createElement('div', {
    key: r.id,
    className: 'cc-card-wrap',
    style: {
    ...
    name: 'cal'
  }), presetParty ? CC_T.bookFor(presetParty) : CC_T.bookNow)))) : null));
}
