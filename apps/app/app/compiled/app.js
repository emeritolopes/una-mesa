...
function parseRouteFromHash() {
  ...
  let q = '';
  try {
    q = decodeURIComponent(rid);
  } catch (e) {
    q = rid;
  }
  if (view === 'detail' && rid) return {
    view: 'detail',
    rid,
    query: '',
    presetTime: null
  };
  if (view === 'booking' && rid) return {
    view: 'booking',
    rid,
    query: '',
    presetTime: null
  };
  if (view === 'results' || view === 'search') return {
    view: 'results',
    rid: null,
    query: q,
    presetTime: null
  };
  if (view === 'concierge') return {
    view: 'concierge',
    rid: null,
    query: q,
    presetTime: null
...
const AP_T = {
  es: {
    ...
    addedFav: '♥ Guardado en favoritos',
    ...
    currentLocationSentinel: 'tu ubicación actual',
    yourArea: 'tu zona',
    countryFallback: 'España'
  },
  en: {
    removedFav: 'Removed from favourites',
    addedFav: '♥ Saved to favourites',
    ...
    currentLocationSentinel: 'your current location',
    yourArea: 'your area',
...
function App() {
  ...
  const [cancelToken, setCancelToken] = useState(() => {
    try {
      return new URLSearchParams(window.location.search).get('cancel_token');
    } catch (_) {
      return null;
    }
  });
  const [connectToken, setConnectToken] = useState(() => {
    try {
      return new URLSearchParams(window.location.search).get('connect_token');
    } catch (_) {
      return null;
    }
  });
  ...
  const [adminVenues, setAdminVenues] = useState(() => {
    try {
      return new URLSearchParams(window.location.search).get('admin') === 'venues';
    } catch (_) {
      return false;
    }
  });
  ...
  useEffect(() => {
    try {
      const t = localStorage.getItem('um-theme');
      if (t) setTheme(t);
      const u = localStorage.getItem('um-app-user');
      if (u) setUser(JSON.parse(u));
      const f = localStorage.getItem('um-app-favs');
      if (f) setFavs(JSON.parse(f));
      const b = localStorage.getItem('um-app-bookings');
      if (b) setBookings(JSON.parse(b));
      const sp = localStorage.getItem('um-spoons');
      if (sp) setSpoons(parseInt(sp, 10) || 0);
    } catch (e) {}
    /* sync dark/light across all Una Mesa pages (live, multi-tab) */
    const onStorage = e => {
      if (e.key === 'um-theme' && e.newValue) {
        animateThemeTo(e.newValue);
  ...
  useEffect(() => {
    try {
      localStorage.setItem('um-app-favs', JSON.stringify(favs));
    } catch (e) {}
  }, [favs]);
  useEffect(() => {
    try {
      localStorage.setItem('um-app-bookings', JSON.stringify(bookings));
    } catch (e) {}
  }, [bookings]);
  useEffect(() => {
    try {
      user ? localStorage.setItem('um-app-user', JSON.stringify(user)) : localStorage.removeItem('um-app-user');
    } catch (e) {}
  }, [user]);
  ...
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(pos => {
      ...
      setGeo({
        status: 'granted',
        label: AP_T.currentLocationSentinel,
        ref: {
          x: 50,
          y: 50
        },
      ...
      fetch(`https://nominatim.openstreetmap.org/reverse?lat=${lat}&lon=${lng}&format=json`).then(r => r.json()).then(d => {
  ...
  const setManualLocation = text => setGeo({
    status: 'manual',
    label: text,
    ref: window.UM_GEOCODE(text)
  });
  const flash = msg => {
    setToast(msg);
  ...
  const toggleTheme = () => {
    const next = theme === 'noche' ? 'crema' : 'noche';
    animateThemeTo(next);
    try {
  ...
  const search = q => setRoute(r => ({
    ...r,
    view: 'results',
    query: q
  }));
  const askConcierge = q => setRoute(r => ({
    ...r,
    view: 'concierge',
    query: q || ''
  }));
  ...
  const awardSpoons = n => {
    setSpoons(s => {
      const t = s + n;
      try {
        localStorage.setItem('um-spoons', String(t));
      } catch (e) {}
      return t;
    });
  };
  const redeemReward = cost => {
    ...
    if (ok) flash(AP_T.rewardRedeemed(cost));
    return ok;
  };
  const onAuth = u => {
    setUser(u);
  ...
  const gateCreateAccount = u => {
    awardSpoons(25);
    flash(AP_T.accountCreated);
    onAuth(u);
  };
  const gateGuest = () => {
    setReserveGate(false);
  ...
  const logout = () => {
    ...
    setUser(null);
    go('home');
    flash(AP_T.loggedOut);
  };
  let screen;
  if (route.view === 'home') screen = React.createElement(window.HomeScreen, {
    go,
    openRest,
    search,
    askConcierge,
    ...
    setManualLocation,
    noRealRestaurants
  });else if (route.view === 'concierge') screen = React.createElement(window.ConciergeScreen, {
    ...
    go,
    user
  });else if (route.view === 'results') screen = React.createElement(window.ResultsScreen, {
    query: route.query,
    openRest,
    favs,
    toggleFav,
    startBook,
    geoLabel: geo.label && geo.label !== AP_T.currentLocationSentinel ? geo.label : AP_T.yourArea,
    geo
  });else if (route.view === 'detail') screen = React.createElement(window.DetailScreen, {
    rid: route.rid,
    back: () => go('results'),
    favs,
    toggleFav,
  ...
  return React.createElement('div', {
    ...
    onTheme: toggleTheme,
    onSearch: search
  }), screen, React.createElement(window.Footer, null), reserveGate ? React.createElement(window.ReserveAuthModal, {
    onClose: () => {
      setReserveGate(false);
      pendingRef.current = null;
    },
    onAccount: gateCreateAccount,
    onGuest: gateGuest
  }) : null, claimOpen ? React.createElement(window.ClaimSpoonsModal, {
    onClose: () => setClaimOpen(false),
    onCreate: claimCreate
  }) : null, authOpen ? React.createElement(window.AuthModal, {
    onClose: () => setAuthOpen(null),
    onAuth,
    geoLabel: geo.label === AP_T.currentLocationSentinel ? AP_T.countryFallback : geo.label,
    initialMode: authOpen
  }) : null, resetPwOpen ? React.createElement(window.ResetPasswordScreen, {
    onDone: () => {
      setResetPwOpen(false);
      go('home');
    }
  }) : null, cancelToken ? React.createElement(window.CancelBookingScreen, {
    token: cancelToken,
    onDone: () => {
      setCancelToken(null);
      try {
  ...
  }) : null, connectToken ? React.createElement(window.StripeConnectScreen, {
    token: connectToken,
    justDone: connectJustDone,
    onDone: () => {
  ...
  }) : null, adminVenues ? React.createElement(window.AdminCreateVenueScreen, {
    onDone: () => {
  ...
  }) : null, React.createElement(window.Toast, {
    msg: toast
  }));
}
