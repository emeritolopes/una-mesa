...
const PR_T = {
  es: {
    ...
    signupSub: 'Personalizamos tus recomendaciones según tu gusto y tu zona.',
    loginSub: 'Entra para reservar y ver tus mesas.',
    createProfileTab: 'Crear perfil',
    signIn: 'Iniciar sesión',
    continueWithGoogle: 'Continuar con Google',
    orCreateWithEmail: 'o crea tu perfil con email',
    ...
    locationPh: 'Tu ciudad o barrio (p. ej. Alicante)',
    ...
    noFavs: 'Sin favoritos todavía. Toca el corazón en cualquier restaurante.',
    explore: 'Explorar',
    cancelBtn: 'Cancelar',
  ...
  en: {
    ...
    signupSub: 'We personalise your recommendations based on your taste and area.',
    loginSub: 'Sign in to book and see your tables.',
    createProfileTab: 'Create profile',
    signIn: 'Sign in',
    continueWithGoogle: 'Continue with Google',
    orCreateWithEmail: 'or create your profile with email',
    ...
    locationPh: 'Your city or neighbourhood (e.g. London)',
    ...
    noFavs: 'No favourites yet. Tap the heart on any restaurant.',
    explore: 'Explore',
    cancelBtn: 'Cancel',
...
function AuthModal({
  ...
  const resetPassword = async () => {
    ...
    setInfo('');
    if (!email.trim()) {
      setError(PR_T.enterEmailForReset);
      return;
    }
  ...
  return React.createElement(Modal, {
    onClose,
    max: isSignup ? 460 : 420
  }, React.createElement('h2', null, isSignup ? PR_T.createProfile : PR_T.welcomeBack), React.createElement('p', {
    className: 'msub'
  }, isSignup ? PR_T.signupSub : PR_T.loginSub), React.createElement('div', {
    className: 'seg'
  }, React.createElement('button', {
    className: isSignup ? 'on' : '',
    onClick: () => setMode('signup')
  }, PR_T.createProfileTab), React.createElement('button', {
    className: !isSignup ? 'on' : '',
    onClick: () => setMode('login')
  }, PR_T.signIn)), React.createElement('div', {
    className: 'sso'
  }, React.createElement('button', {
    className: 'sso-btn',
    onClick: () => {
    ...
    name: 'google'
  }), PR_T.continueWithGoogle)), React.createElement('div', {
    className: 'divider'
  }, isSignup ? PR_T.orCreateWithEmail : PR_T.orWithEmail), React.createElement('form', {
    onSubmit: submit
  }, isSignup ? React.createElement('div', {
    className: 'field'
  }, React.createElement('label', null, PR_T.nameLabel), React.createElement('input', {
    value: name,
    onChange: e => setName(e.target.value),
    placeholder: PR_T.namePh,
    required: true,
    disabled: loading
  })) : null, React.createElement('div', {
    className: 'field'
  }, React.createElement('label', null, PR_T.emailLabel), React.createElement('input', {
    type: 'email',
    value: email,
    onChange: e => setEmail(e.target.value),
    placeholder: PR_T.emailPh,
    required: true,
    disabled: loading
  })), React.createElement('div', {
    className: 'field'
  }, React.createElement('label', null, PR_T.passwordLabel), React.createElement('input', {
    type: 'password',
    value: password,
    ...
    className: 'field'
  }, React.createElement('label', null, PR_T.locationLabel), React.createElement('input', {
    value: loc,
    onChange: e => setLoc(e.target.value),
    placeholder: PR_T.locationPh
  })) : null, isSignup ? React.createElement('div', {
    className: 'field'
  }, React.createElement('label', null, PR_T.cuisinePrefsLabel), React.createElement('div', {
    className: 'pref-chips'
  }, CUISINE_PREFS.map(p => React.createElement('button', {
    key: p,
    type: 'button',
    className: 'chip' + (prefs.includes(p) ? ' on' : ''),
    onClick: () => togglePref(p)
  }, p)))) : null, error ? React.createElement('p', {
    style: {
      color: '#dc2626',
      fontSize: 13,
      margin: '10px 0 0',
  ...
  }, error) : null, info ? React.createElement('p', {
    style: {
      color: '#16a34a',
      fontSize: 13,
      margin: '10px 0 0',
  ...
  }, info) : null, React.createElement('button', {
    ...
    disabled: loading
  }, loading ? isSignup ? PR_T.creatingAccount : PR_T.signingIn : isSignup ? PR_T.createProfileBtn : PR_T.enterBtn), !isSignup ? React.createElement('button', {
    type: 'button',
...
function ProfileScreen({
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        let query = sb.from('reservations').select('*, venues(name, address, photo_url, cuisine, deposit_amount, currency)').in('status', ['confirmed', 'unconfirmed', 'pending']).order('date', {
          ascending: false
        });
        if (authUser?.id) {
        ...
        if (error) {
          console.warn('[UNA MESA] loadBookings:', error.message);
          setSupaBookings([]);
          return;
        }
  ...
  const BookingRow = (b, isPast) => {
    return React.createElement('div', {
      ...
      style: {
        ...
        height: 56,
        borderRadius: 12,
        background: 'linear-gradient(150deg, #2D2420, #4A3728)',
        flexShrink: 0
      }
    }), React.createElement('div', {
      className: 'br-main'
      ...
      className: 'br-meta'
    }, (b.dayLabel || PR_T.today) + ' · ' + b.time + ' · ' + b.party + (b.party === 1 ? PR_T.guestSingular : PR_T.guestPlural)), React.createElement('div', {
      className: 'br-meta',
      style: {
        marginTop: '2px'
      }
  ...
  }, PR_T.goBack), React.createElement('button', {
    onClick: () => {
      doCancel(cancelTarget);
      setCancelTarget(null);
    },
    style: {
      padding: '10px 24px',
  ...
  let panel;
  if (tab === 'reservas') {
    panel = upcoming.length || past.length ? React.createElement('div', null, upcoming.length ? React.createElement('div', null, React.createElement('div', {
      className: 'prof-sec-label'
    }, PR_T.upcoming), upcoming.map(b => BookingRow(b, false))) : null, past.length ? React.createElement('div', {
      style: {
        marginTop: '22px'
      }
    }, React.createElement('div', {
      className: 'prof-sec-label'
    }, PR_T.history), past.map(b => BookingRow(b, true))) : null) : React.createElement('div', {
      className: 'empty'
    }, React.createElement(Icon, {
      name: 'cal'
    }), React.createElement('p', null, PR_T.noBookings), React.createElement('button', {
      className: 'btn btn-acc',
      style: {
        marginTop: '14px'
      },
      onClick: () => go('results')
    }, PR_T.findRestaurants));
  } else if (tab === 'favoritos') {
    panel = favList.length ? React.createElement('div', {
      className: 'rgrid'
    }, favList.map(r => React.createElement(RestaurantCard, {
      key: r.id,
      r,
      fav: true,
      onFav: toggleFav,
      ...
      name: 'heart'
    }), React.createElement('p', null, PR_T.noFavs), React.createElement('button', {
      className: 'btn btn-acc',
      style: {
        marginTop: '14px'
      },
    ...
    panel = React.createElement('div', null, React.createElement(window.GastroPassport, {
      data,
      bookings,
      openRest,
      go
    ...
    }, React.createElement(window.PalateAI, {
      user,
      data,
      bookings,
      favs
    ...
    panel = React.createElement(window.RewardsMarket, {
      spoons,
      onRedeem
    });
  }
  ...
  }, PR_T.deleteModalTitle), React.createElement('p', {
    style: {
      fontSize: 13,
      color: '#6B7280',
      lineHeight: 1.6,
  ...
  }, PR_T.deleteModalBody), React.createElement('div', {
    style: {
      display: 'flex',
      gap: 10
    }
  ...
  }, PR_T.cancelBtn), React.createElement('button', {
  ...
  return React.createElement(React.Fragment, null, cancelModal, deleteModal, React.createElement('div', {
    className: 'view'
  }, React.createElement('div', {
    className: 'wrap'
  }, React.createElement('div', {
    ...
    className: 'prof-av'
  }, formatName(user)[0].toUpperCase()), React.createElement('div', null, React.createElement('div', {
    ...
    className: 'll'
  }, PR_T.loyaltyProgram), React.createElement('div', {
    className: 'lv'
  }, points + PR_T.pts), React.createElement('div', {
    className: 'lbar'
  }, React.createElement('i', {
    style: {
      width: Math.max(8, points) + '%'
    }
  })), React.createElement('div', {
    className: 'lnext'
  }, PR_T.visitsToNext(toNext)))), React.createElement('div', {
    className: 'tabs'
  }, React.createElement('div', {
    className: 'tab' + (tab === 'reservas' ? ' on' : ''),
    onClick: () => setTab('reservas')
  }, PR_T.tabReservas), React.createElement('div', {
    className: 'tab' + (tab === 'pasaporte' ? ' on' : ''),
    onClick: () => setTab('pasaporte')
  ...
  }, PR_T.tabRewards)), panel, React.createElement('div', {
    style: {
      marginTop: '32px',
      paddingTop: '24px',
  ...
  }, PR_T.dangerZone), React.createElement('button', {
