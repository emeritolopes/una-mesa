...
function Icon({
  ...
  }, parts.map((d, i) => React.createElement('path', {
    key: i,
    d
  })));
}
...
const CM_T = {
  es: {
    ...
    forRestaurants: '¿Tienes un restaurante?',
    searchPh: 'Busca restaurante, cocina o zona…',
    search: 'Buscar',
  ...
  en: {
    ...
    forRestaurants: 'Own a restaurant?',
    searchPh: 'Search restaurant, cuisine or area…',
    search: 'Search',
...
function Header({
  ...
  React.createElement('div', {
    ...
    title: CM_T.search,
    onClick: () => {
      if (scrolled && inputRef.current) inputRef.current.focus();else go('results');
    }
  }, React.createElement(Icon, {
    name: 'search'
  })), React.createElement('button', {
    className: 'icon-btn',
    title: theme === 'noche' ? CM_T.lightMode : CM_T.darkMode,
    onClick: onTheme
  }, React.createElement(Icon, {
    name: theme === 'noche' ? 'sun' : 'moon'
    ...
    className: 'av'
  }, formatName(user)[0].toUpperCase()), React.createElement('span', {
    className: 'nm'
  }, formatName(user).split(' ')[0])), avatarOpen && React.createElement('div', {
    style: {
      position: 'absolute',
      top: 'calc(100% + 8px)',
      right: 0,
  ...
  }, CM_T.myProfile), React.createElement('div', {
    style: {
      height: 1,
      background: 'var(--line)',
      margin: '0 12px'
  ...
  }, CM_T.logout))) : React.createElement(React.Fragment, null, React.createElement('button', {
    className: 'btn btn-ghost btn-sm hdr-signin',
    onClick: () => onAuth('login')
  }, CM_T.signIn), React.createElement('button', {
    className: 'btn btn-acc btn-sm',
    onClick: () => onAuth('register')
  }, CM_T.createProfile))), /* ── Hamburger (mobile only) ── */
  React.createElement('button', {
    type: 'button',
    className: 'hdr-burger' + (menuOpen ? ' open' : ''),
    onClick: () => setMenuOpen(o => !o),
    'aria-label': menuOpen ? CM_T.closeMenu : CM_T.openMenu
  }, React.createElement('span', null), React.createElement('span', null), React.createElement('span', null)))), /* ── Panel de navegación móvil ── */
  React.createElement('nav', {
...
const SOCIAL_LINKS = [{
  name: 'instagram',
  href: 'https://www.instagram.com/unamesagroup/',
  label: 'Instagram'
}, {
  ...
  name: 'linkedin',
  href: 'https://www.linkedin.com/company/138484148/',
  label: 'LinkedIn'
}, {
  name: 'facebook',
  href: 'https://www.facebook.com/profile.php?id=61592830190355',
  label: 'Facebook'
}, {
  name: 'tiktok',
  href: 'https://www.tiktok.com/@unamesagroup',
  label: 'TikTok'
}];
function Footer() {
