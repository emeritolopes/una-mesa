...
function MenuVideoScreen({ rid, back }) {
  const lang = mvMarket();
  const t = MV_T[lang];
  ...
  const shareVideo = (v, restaurantName) => {
    ...
    if (navigator.share) {
      navigator.share({ title: v.dish_name, text: shareText, url: v.video_url }).catch(() => {});
    } else {
  ...
  useEffect(() => {
    if (!r || !r.menuVideoAccess || !window.UMAuth) { setVideos([]); return; }
    window.UMAuth.sb.from('menu_videos').select('*').eq('venue_id', rid).order('sort_order')
      .then(({ data: rows }) => setVideos(rows || []));
  }, [rid]);

  if (!r) {
    return React.createElement('div', { className:'wrap', style:{ padding:'60px 0' } },
      React.createElement('button', { onClick: back, style:{ marginBottom:20 } }, t.back),
      React.createElement('p', null, t.notAvailable)
    );
  }

  if (!r.menuVideoAccess) {
    return React.createElement('div', { className:'wrap', style:{ padding:'60px 0', textAlign:'center' } },
      React.createElement('button', { onClick: back, style:{ marginBottom:20 } }, t.back),
      React.createElement('p', null, t.notAvailable)
    );
  }
  ...
  const categoryOrder = [];
  (videos || []).forEach(v => {
    if (!byCategory[v.category]) { byCategory[v.category] = []; categoryOrder.push(v.category); }
  ...
  return React.createElement('div', { className:'wrap', style:{ padding:'40px 0 80px' } },
    ...
    videos === null
      ...
      : videos.length === 0
      ? React.createElement('p', { className:'muted' }, t.empty)
      : categoryOrder.map(cat =>
          React.createElement('div', { key:cat, style:{ marginBottom:40 } },
            ...
            React.createElement('div', { style:{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(220px, 1fr))', gap:20 } },
              byCategory[cat].map(v =>
                React.createElement('div', { key:v.id, style:{ borderRadius:12, overflow:'hidden', background:'var(--bg-3)' } },
                  ...
                  React.createElement('div', { style:{ padding:'10px 12px', display:'flex', alignItems:'center', justifyContent:'space-between', gap:8 } },
                    React.createElement('p', { style:{ fontWeight:600, fontSize:14 } }, v.dish_name),
                    React.createElement('button', {
                      type:'button',
                      onClick: () => shareVideo(v, r.name),
                      title: t.share,
                      style:{ background:'none', border:'none', cursor:'pointer', color:'var(--muted)', padding:4, flexShrink:0 }
                    }, React.createElement(Icon, { name:'share', style:{ width:19, height:19 } }))
                  )
                )
              )
            )
